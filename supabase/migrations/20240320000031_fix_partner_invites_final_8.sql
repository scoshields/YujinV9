...
CREATE POLICY "Enable insert for authenticated users" ON workout_partners
  FOR INSERT WITH CHECK (
    ...
    NOT EXISTS ( -- Prevent duplicate invites
      ...
      WHERE (
        (wp.user_id = user_id AND wp.partner_id = partner_id) OR
        (wp.partner_id = user_id AND wp.user_id = partner_id)
      )
    )
  );
...
DROP CONSTRAINT IF EXISTS workout_partners_unique_partnership;

CREATE UNIQUE INDEX workout_partners_unique_partnership
ON workout_partners ((LEAST(user_id, partner_id)), (GREATEST(user_id, partner_id)));